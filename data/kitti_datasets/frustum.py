...
from data.kitti_datasets.attributes import kitti_attributes as kitti
from data.kitti_datasets.container import G
from data.kitti_datasets.config import configs
...
class FrustumKittiDataset(Dataset):
    def __init__(self, split, num_points, classes, num_heading_angle_bins,class_name_to_size_template_id, from_rgb_detection,
                 random_flip, random_shift, frustum_rotate):
        """
        ...
        """
        BASE_DIR = os.path.sep + os.path.join('material','data','semantic_kitti')
        self.root = os.path.join(BASE_DIR, 'kitti','frustum','frustum_data')
        self.split = split
...
if __name__ == "__main__":
    train_loader = DataLoader(FrustumKittiDataset(split='train', num_points=10000, classes=configs.classes,
    ...
    for data,targets in train_loader:
        print('data: ', data['features'].size(), 'type: ', data['features'].type())
        #print('targets: ', targets.size(), 'type: ', targets.type())
        
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        data = data['features'][25]
        ...
        ax.scatter(X,Y,Z, c=C, cmap="inferno", s=1)
